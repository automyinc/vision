...
const vnx::Hash64 PointCloud::VNX_TYPE_HASH(0x39f95165fe1a99full);
const vnx::Hash64 PointCloud::VNX_CODE_HASH(0xd2d99a040f6fe453ull);
...
void PointCloud::accept(vnx::Visitor& _visitor) const {
	...
	_visitor.type_field(_type_code->fields[3], 3); vnx::accept(_visitor, odometry);
	_visitor.type_field(_type_code->fields[4], 4); vnx::accept(_visitor, points);
...
void PointCloud::write(std::ostream& _out) const {
	...
	_out << ", \"odometry\": "; vnx::write(_out, odometry);
...
void PointCloud::read(std::istream& _in) {
	...
	for(const auto& _entry : _object) {
		...
		} else if(_entry.first == "odometry") {
			vnx::from_string(_entry.second, odometry);
...
const vnx::TypeCode* PointCloud::get_type_code() {
	const vnx::TypeCode* type_code = vnx::get_type_code(vnx::Hash64(0x39f95165fe1a99full));
	if(!type_code) {
...
std::shared_ptr<vnx::TypeCode> PointCloud::create_type_code() {
	...
	type_code->name = "vision.PointCloud";
	type_code->type_hash = vnx::Hash64(0x39f95165fe1a99full);
	type_code->code_hash = vnx::Hash64(0xd2d99a040f6fe453ull);
	type_code->is_class = true;
	type_code->create_value = []() -> std::shared_ptr<vnx::Value> { return std::make_shared<PointCloud>(); };
	type_code->depends.resize(2);
	type_code->depends[0] = ::vision::CameraInfo::get_type_code();
	type_code->depends[1] = ::vision::point_t::get_type_code();
...
void read(TypeInput& in, ::vision::PointCloud& value, const TypeCode* type_code, const uint16_t* code) {
	...
	for(const vnx::TypeField* _field : type_code->ext_fields) {
		switch(_field->native_index) {
			...
			case 3: vnx::read(in, value.odometry, type_code, _field->code.data()); break;
			case 4: vnx::read(in, value.points, type_code, _field->code.data()); break;
...
void write(TypeOutput& out, const ::vision::PointCloud& value, const TypeCode* type_code, const uint16_t* code) {
	if(!type_code || (code && code[0] == CODE_ANY)) {
		type_code = vnx::write_type_code<::vision::PointCloud>(out);
		vnx::write_class_header<::vision::PointCloud>(out);
	}
	...
	vnx::write(out, value.odometry, type_code, type_code->fields[3].code.data());
	vnx::write(out, value.points, type_code, type_code->fields[4].code.data());
}
